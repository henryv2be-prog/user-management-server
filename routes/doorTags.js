...
router.post('/', authenticate, requireAdmin, validateDoorTag, async (req, res) => {
  try {
    ...
    // Ensure door_tags table exists - simplified approach
    console.log('Ensuring door_tags table exists...');
    const sqlite3 = require('sqlite3').verbose();
    const path = require('path');
    const DB_PATH = process.env.DB_PATH || path.join(__dirname, '..', 'database', 'users.db');
    
    await new Promise((resolve, reject) => {
      console.log('Creating/verifying door_tags table with DB_PATH:', DB_PATH);
      const db = new sqlite3.Database(DB_PATH, (openErr) => {
        if (openErr) {
          console.error('Error opening database:', openErr);
          reject(openErr);
        ...
        console.log('Database opened successfully for table creation');
      });
      ...
      db.run(`CREATE TABLE IF NOT EXISTS door_tags (
        ...
        if (err) {
          ...
          console.log('door_tags table created/verified successfully');
          db.close();
          resolve();
        }
      });
    ...
    await new Promise((resolve, reject) => {
      ...
      db.run(`CREATE INDEX IF NOT EXISTS idx_door_tags_door_id ON door_tags(door_id)`, (err) => {
        if (err) {
          ...
          console.log('door_id index created/verified');
        }
        db.close();
        resolve();
      });
    ...
    await new Promise((resolve, reject) => {
      ...
      db.run(`CREATE INDEX IF NOT EXISTS idx_door_tags_tag_id ON door_tags(tag_id)`, (err) => {
        if (err) {
          ...
          console.log('tag_id index created/verified');
        }
        db.close();
        resolve();
      });
    ...
    console.log('Table and indexes verified, checking for existing tag...');
