...
// ESP32 Tag-based Access Request endpoint (for QR code/NFC scanning)
router.post('/access/tag', async (req, res) => {
  try {
    const { doorId, tagId } = req.body;
    
    if (!doorId || !tagId) {
      return res.status(400).json({
        error: 'Bad Request',
        message: 'doorId and tagId are required'
      });
    }
    ...
    if (!door.isOnline) {
      return res.status(503).json({
        error: 'Door Offline',
        message: 'Door is currently offline',
        accessGranted: false
      });
    }
    
    // Find the tag and get associated user
    ...
    if (!doorTag) {
      return res.status(404).json({
        error: 'Tag Not Found',
        message: 'Tag not found or not associated with any door',
        accessGranted: false
      });
    }
    
    // Check if tag is associated with this door
    if (doorTag.doorId !== parseInt(doorId)) {
      return res.status(403).json({
        error: 'Tag Not Authorized',
        message: 'Tag is not authorized for this door',
        accessGranted: false
      });
    }
    
    // For now, we'll need to associate tags with users
    // This is a simplified version - you may need to extend the door_tags table
    // to include user_id or create a separate user_tags table
    
    // For demonstration, we'll return success if the tag is valid for the door
    // In a real implementation, you'd check if the user associated with the tag has access
    
    // Log the access attempt
    await EventLogger.log(req, 'access', 'granted', 'door', door.id, door.name, 
      `Access granted via tag ${tagId}`);
    
    // Send door control command
    const doorControlSuccess = await door.sendCommand('unlock');
    
    res.json({
      success: true,
      message: 'Access granted via tag',
      accessGranted: true,
      doorControlSuccess,
      tagId,
      doorId
    ...
    console.error('Tag access request error:', error);
    res.status(500).json({
      error: 'Internal Server Error',
      message: 'Failed to process tag access request',
      accessGranted: false
    });
  }
});
